...
const preferredEncodings = encodingDefaultPreference.filter(encoding => encodingMethods[encoding] || encoding === 'identity')
...
module.exports = (options = {}) => {
  let { filter = compressible, threshold = 1024, defaultEncoding, wildcardAcceptEncoding, encodingPreference = encodingDefaultPreference } = options
  if (typeof threshold === 'string') threshold = bytes(threshold)
  if (!wildcardAcceptEncoding || wildcardAcceptEncoding === '*') wildcardAcceptEncoding = 'gzip'
  defaultEncoding = defaultEncoding === '*' ? wildcardAcceptEncoding : !defaultEncoding ? 'identity' : defaultEncoding
  ...
  async function compressMiddleware (ctx, next) {
    ...
    const finalEncodings = compressMiddleware.preferredEncodings.filter((encoding) => responseOptions[encoding] !== false && responseOptions[encoding] !== null)
    ...
    const encoding = negotiator.encoding(finalEncodings, { preferred: encodingPreference })
