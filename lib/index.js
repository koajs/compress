...
module.exports = (options = {}) => {
  ...
  const encodings = preferredEncodings.filter((encoding) => options[encoding] !== false)
  ...
  return async (ctx, next) => {
    ...
    const encodingWeights = parseAcceptEncoding(ctx.request.headers['accept-encoding'])
    const encoding = getPreferredContentEncoding(encodingWeights, encodings)
