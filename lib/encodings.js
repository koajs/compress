...
class Encodings {
  ...
  getPreferredContentEncoding () {
    ...
    if (encodingWeights.get('identity') === 0) throw errors(406, 'Please accept zstd, br, gzip, deflate, or identity.')
...
Encodings.preferredEncodings = ['zstd', 'br', 'gzip', 'deflate']
Encodings.reDirective = /^(?:\s*(zstd|br|gzip|compress|deflate|identity|\*)\s*(?:;\s*q\s*=\s*(\d(?:\.\d)?))?\s*)?$/
